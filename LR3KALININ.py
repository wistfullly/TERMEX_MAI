...
# система дифференциальных уравнений для вычисления законов движения от t
...
dphi = Y[:, 2]
dpsi = Y[:, 3]
ddphi = [odesys(y0, t, m1, m2, m3, l, r, c)[2] for y0, t in zip(Y, t)]
ddpsi = [odesys(y0, t, m1, m2, m3, l, r, c)[3] for y0, t in zip(Y, t)]

Rx = -(m1 / 2 + m2 + m3) * l * (ddphi * np.sin(phi) + dphi ** 2 * np.cos(phi)) + m3 * r * (
            ddpsi * np.sin(psi) - psi ** 2 * np.sin(psi))
Ry = (m1 / 2 + m2 + m3) * l * (ddphi * np.sin(phi) + dphi ** 2 * np.cos(phi)) + m3 * r * (
            ddpsi * np.sin(psi) - psi ** 2 * np.sin(psi))
